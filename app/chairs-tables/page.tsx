  ...
  return (
    <>
      <SlideshowHeader
        images={[]}
        emoji="🪑"
        title={<>Chairs <span className="text-brand-yellow">&amp; Tables</span></>}
        subtitle="Everything you need to seat your guests in comfort and style."
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {chairsTablesEquipment.map((item) => (
              <EquipmentCard key={item.name} {...item} category="chairs-tables" />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-20" id="book">
        <div className="max-w-2xl mx-auto px-4">
          ...
          <InquiryForm defaultService="Chairs & Tables" />
        </div>
      </section>
    </>
  );
